...
static void compute_cd(envg_list_t **head, char **parsed_input,
                        const int ac, bool_t *success)
{
    ...
    if (ac == 1 || !my_strcmp(parsed_input[1], "~", 0)) {
        ...
        if (!home) {
            *success = FALSE;
            ...
            change_working_dir(home->var_value, 1, success);
    } else if (!my_strcmp(parsed_input[1], "-", 0)) {
        last_dir = is_var_in_env(head, "OLDPWD");
        if (last_dir)
            change_working_dir(last_dir->var_value, 0, success);
        else {
            *success = FALSE;
            my_printf(": %s.\n", strerror(ENOENT));
        }
    } else
        change_working_dir(parsed_input[1], 0, success);
}
...
void my_cd(envg_list_t **head, char **parsed_input, bool_t *success)
{
    ...
    compute_cd(head, parsed_input, ac, success);
    new_cur_pwd = get_current_pwd();
    my_setenv(head, (char *[]){"setenv", "OLDPWD", oldpwd, NULL}, success);
    if (oldpwd)
        free(oldpwd);
    my_setenv(head, (char *[]){"setenv", "PWD", new_cur_pwd, NULL}, success);
    if (new_cur_pwd)
        free(new_cur_pwd);
}